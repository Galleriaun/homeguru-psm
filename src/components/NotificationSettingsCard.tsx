...
/**
 * Per-device notification toggles, shown as a card on the Profil page (moved
 * here from the old bell-icon modal). Each row is an event type the push
 * pipeline supports; toggling a row upserts a notification_preferences row (RLS
 * scopes it to the current user). Defaults are ON server-side, so an untouched
 * preference reads as enabled. Only the events this role can actually receive a
 * push for are shown (mirrors the trigger recipient lists).
 *
 * `bare` renders the content without the outer Card, so it can share a single
 * card with the push-enable section on the Profil page.
 */
export function NotificationSettingsCard({ bare = false }: { bare?: boolean }) {
  ...
  const content = (
    <>
      ...
      {!loadError && prefs === null && (
        <p className="mt-3 text-sm text-stone-600 dark:text-stone-300">Yükleniyor…</p>
      )}
