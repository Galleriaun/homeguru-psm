-- =============================================================================
-- HomeGuru PMS — migration 125
-- Düzenli giderler skip the onay queue, and a non-approved template never posts.
-- =============================================================================
-- Two defects, one root cause: onay only ever gated a düzenli gider's FIRST
-- month, and the cron ignored the verdict entirely.
--
...
--     (106/124) — no onay, ever. So a yönetici had to approve month 1 while
...
--     raw role. That matters: auth_can_review_region (096) requires
--     PROPERTY_MANAGER to have a non-NULL auth_region(), so a region-less Alt
--     Yönetici cannot review; gating on the role would make approve_expense
--     RAISE and abort the whole creation. "If you could approve it yourself, we
--     skip the click" — no new privilege is granted to anyone.
--     A Personel's (YETKILI / PERSONEL_BORNOVA) düzenli still goes to onay once;
--     the cron takes over only after it is approved.
...
--
-- Apply AFTER 124 (this re-declares generate_recurring_expenses on top of it,
-- keeping 124's `>=` start-month guard).
-- =============================================================================
...
--    when it is about to self-approve; identical to 055 otherwise.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. generate_recurring_expenses — 124's version plus: only APPROVED templates
--    materialise. A rejected template must never charge the kasa again, and a
--    pending one must wait for its onay.
-- -----------------------------------------------------------------------------
